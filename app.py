...
<style>
  ...
  [data-testid="stMetricValue"] { font-family:'Courier New',Courier,monospace !important; font-size:20px !important; font-weight:700 !important; }
  [data-testid="stMetricLabel"] { font-size:11px !important; color:#64748B !important; }
  .stTabs [data-baseweb="tab"] { background:rgba(255,255,255,0.04); border-radius:8px; padding:6px 18px; border:none !important; color:#64748B; font-size:12px; font-weight:600; }
...
def process_file(file_bytes: bytes):
    ...
    return (df, kpi_summary(df), cat, seller_agg(df), lang_agg(df),
            funnel_df(df), top_products(df, top_n=50), cvr_segments(cat))
...
with st.sidebar:
    ...
    uploaded_file = st.file_uploader(
        "📂 Upload GA4 CSV",
        type=["csv"],
        help="Expected: sku, seller_type, lang_code, attribute_set, views_total, "
             "Items added to cart, Items checked out, orders_count, qty, revenue, conversion_rate_orders",
    )

    # Persist last uploaded file across refreshes
    if uploaded_file is not None:
        st.session_state["last_file_bytes"] = uploaded_file.read()
    ...
    st.markdown("### 🔍 Filters")
    seller_sel = st.selectbox("Seller Type", ["All"] + sorted(df["seller_type"].dropna().unique().tolist()))
    lang_sel   = st.selectbox("Language",    ["All"] + sorted(df["lang_code"].dropna().unique().tolist()))
...
# ── Apply filters ────────────────────────────────
filtered_df = df.copy()
if seller_sel != "All": filtered_df = filtered_df[filtered_df["seller_type"] == seller_sel]
if lang_sel   != "All": filtered_df = filtered_df[filtered_df["lang_code"]   == lang_sel]

f_kpi = kpi_summary(filtered_df)
f_cat = category_agg(filtered_df, top_n=20)
if cat_sel: f_cat = f_cat[f_cat["category"].isin(cat_sel)]
f_fun = funnel_df(filtered_df)

# ── Page header ──────────────────────────────────
st.markdown("""
<div style="padding:16px 0 12px; border-bottom:1px solid rgba(255,255,255,0.07); margin-bottom:20px;
...
if seller_sel != "All" or lang_sel != "All":
    badges = ([f"🏪 {seller_sel}"] if seller_sel != "All" else []) + ([f"🌐 {lang_sel}"] if lang_sel != "All" else [])
...
with tab1:
    ...
    with col_a:
        ...
        apply_theme(fig, height=380); st.plotly_chart(fig, use_container_width=True)
    ...
    with col_c:
        ...
        apply_theme(fig2, height=320); st.plotly_chart(fig2, use_container_width=True)
    ...
    with col_d:
        ...
        apply_theme(fig3, height=320); st.plotly_chart(fig3, use_container_width=True)

# ── TAB 2: CATEGORIES ───────────────────────────
with tab2:
    st.subheader("Category Performance Table")
    dc = {"category":"Category","revenue":"Revenue (EGP)","orders":"Orders","views":"Views",
    ...
    with ce:
        ...
        apply_theme(fig4, height=380, showlegend=False); st.plotly_chart(fig4, use_container_width=True)
    ...
    with cf:
        ...
        apply_theme(fig5, height=380, showlegend=False); st.plotly_chart(fig5, use_container_width=True)
    ...
    apply_theme(fig6, height=420); st.plotly_chart(fig6, use_container_width=True)
...
with tab3:
    ...
    with ch:
        ...
        apply_theme(fdrop, height=420); st.plotly_chart(fdrop, use_container_width=True)
    ...
    st.markdown("---")
    lc, rc = st.columns(2)
    with lc: st.warning(f"⚠️ **Cart Abandonment:** {f_kpi['total_cart']-f_kpi['total_checkout']:,} users didn't reach checkout.")
    with rc: st.error(f"🚨 **Checkout Drop-off:** {f_kpi['total_checkout']-f_kpi['total_orders']:,} users didn't order. Rate: {f_kpi['cvr_checkout_order']}%")
...
with tab4:
    ...
    with ci:
        ...
        fs2.add_trace(go.Bar(name="Revenue", x=sel["seller_type"], y=sel["revenue"],
                             marker_color=["#F97316","#3B82F6"],
                             text=sel["revenue"].apply(lambda v: f"EGP {v/1e6:.1f}M"), textposition="outside"))
        fs2.add_trace(go.Scatter(name="Orders", x=sel["seller_type"], y=sel["orders"],
                                 mode="markers+lines", marker=dict(size=12, color="#10B981"),
                                 line=dict(color="#10B981", dash="dot")), secondary_y=True)
        fs2.update_layout(**{**PLOT_LAYOUT,"height":320}); st.plotly_chart(fs2, use_container_width=True)
    ...
    with cj:
        ...
        apply_theme(fl2, height=320, showlegend=False); st.plotly_chart(fl2, use_container_width=True)
    ...
    # FIX: assign columns to named vars — no [i%2] indexing
...
with tab5:
    ...
    with ck:
        ...
        apply_theme(ft, height=480); st.plotly_chart(ft, use_container_width=True)
    ...
    with cl:
        ...
        apply_theme(fcp, height=480, margin=dict(l=0,r=0,t=10,b=0)); st.plotly_chart(fcp, use_container_width=True)

st.markdown('<div style="text-align:center;color:#334155;font-size:11px;margin-top:40px;padding:20px 0;'
